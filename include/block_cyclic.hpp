    ...
    namespace dev {
        ...
        namespace internal {
            ...
            auto create_blocks(const Dimensions<N> &num_values, const Dimensions<N> &block_size,
                ...
                for (int i = 0; i < N; ++i) {
                    blocks[i] =
                            create_blocks(num_values[i], block_size[i], processor_dimensions[i]);
                }
                return blocks;
            }
        }// namespace internal
        ...
        BlockCyclic<N>::BlockCyclic(const Dimensions<N> &num_global_values,
            ...
            : _num_global_values(num_global_values), _block_sizes(block_sizes),
              ...
              _grid_layout(
                      {internal::create_blocks(num_global_values, block_sizes, processor_grid)}) {}
        ...
        auto make_block_wise_distribution(const Dimensions<N> &num_global_values,
            ...
            for (int i = 0; i < N; ++i) {
                ...
                block_sizes[i] =
                        std::ceil(static_cast<double>(num_global_values[i]) / num_processors);
            }
