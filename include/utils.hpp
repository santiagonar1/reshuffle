...
namespace reshuffle::internal {
    ...
    inline auto have_same_num_values(const BlockCyclic &first, const BlockCyclic &second) -> bool {
        return first.get_num_values() == second.get_num_values();
    }

    inline auto have_same_num_values(const std::array<BlockCyclic, 2> &first,
