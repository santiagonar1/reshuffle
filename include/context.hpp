...
namespace reshuffle {
    ...
    class Context {
    public:
        Context(const distribution::DataDistribution<N> &distribution, const MPI_Comm &comm);
        ...
        [[nodiscard]] auto get_distribution() const -> const distribution::DataDistribution<N> &;

        auto operator==(const Context &other) const -> bool;

    private:
        const std::unique_ptr<const distribution::DataDistribution<N>> _distribution;
    ...
    template<std::size_t N>
    Context<N>::Context(const distribution::DataDistribution<N> &distribution, const MPI_Comm &comm)
    ...
    template<std::size_t N>
    auto Context<N>::get_distribution() const -> const distribution::DataDistribution<N> & {
